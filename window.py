...
class Window():
    ...
    def __init__(self, master, algorithm, maze_size):
        ...
        self._canvas = tk.Canvas(
            self._master, width=self._width, height=self._height, background="black")
        self._canvas.grid(row=0, column=0, sticky=tk.N)

        self._score_label = tk.Label(
            self._master, text='0', font=('Arial', 20))
        self._level_label = tk.Label(
            self._master, text='0', font=('Arial', 20))
        self._lives_label = tk.Label(
            self._master, text='0', font=('Arial', 20))
    ...
    def display_completed(self) -> None:
        ...
        # bindings are disabled during loading screen
    ...
    def _gameover_transition(self) -> None:
        # bindings are disabled when game is over
        self._bindings_enabled(False)
        # pacman is no longer on the board, so no image required
    ...
    def run(self) -> None:
        self.delay_beginning()
        # put again here to allow mainloop() to still occur and also call gameloop
        self._master.after(2000, self.update)
