...
class Agent(Character):
    ...
    def random_choice(self) -> int or float:  # type: ignore
    ...
    def path_finding_direction(self, path):
        ...
        if self.not_empty_path(path):
            ...
            if self.algorithm == "BFS" or not self.invulnerable:
    ...
    def astar_search(self, board, start, endpoint_y, endpoint_x) -> deque:
        ...
        came_from = {}
        open_list = [((self.y, self.x), 0)]  # Priority queue (node, f_score)
        # Cost from start node to each explored node
        g_score = {(self.y, self.x): 0}

        heuristics = board.generate_heuristics()
            ...
            for neighbor in board.get_valid_neighbors(current[0], current[1]):
                ...
                if neighbor not in g_score or tentative_g_score < g_score[neighbor]:
                    ...
                    f_score = tentative_g_score * \
                        heuristics[neighbor[0]][neighbor[1]]
    ...
    def dijkstra_search(self, board, start, endpoint_y, endpoint_x) -> deque:
        ...
        came_from = {}
        open_set = {((self.y, self.x), 0)}  # Priority queue (node, cost)
        # Cost from start node to each explored node
        g_score = {(self.y, self.x): 0}
        ...
        while open_set:
            ...
            for neighbor in board.get_valid_neighbors(current[0], current[1]):
                # Assuming uniform edge cost of 1
                tentative_g_score = g_score[current] + 1
