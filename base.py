...
def next_turn(mainagent, food):

    x, y = mainagent.coordinates[0]
    ...
    mainagent.coordinates.insert(0, (x, y))

    square = canvas.create_rectangle(
        x, y, x + SPACE_SIZE,
        y + SPACE_SIZE, fill=SNAKE)

    mainagent.squares.insert(0, square)
    ...
    if x == food.coordinates[0] and y == food.coordinates[1]:
        ...
        del mainagent.coordinates[-1]

        canvas.delete(mainagent.squares[-1])

        del mainagent.squares[-1]

    if check_collisions(mainagent):
        game_over()

    else:
        window.after(SPEED, next_turn, mainagent, food)
...
def check_collisions(mainagent):
    ...
    for body_part in mainagent.coordinates[1:]:
